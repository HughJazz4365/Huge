...
pub const frmt = @import("format.zig");

//generated by: https://github.com/Snektron/vulkan-zig
const vk = @import("vk.zig");

pub const device_allocator = @import("vulkanDeviceMemoryAllocator.zig");
...
pub const max_descriptors_of_type = @as(comptime_int, @intCast(std.math.powi(u32, 2, 20) catch unreachable));
const max_image_staging_buffer_count = 8;
...
const bindless_descriptor_types = [_]vk.DescriptorType{ .storage_buffer, .combined_image_sampler };
...
var image_staging_buffers: [max_image_staging_buffer_count]VKBuffer = undefined;
var image_staging_buffer_count: usize = 0;
...
pub fn cmdPushConstant(
    ...
    switch (T) {
        ...
        VKBuffer, VKTexture => cmdPushConstantBytes(cmd, pipeline, name, @ptrCast(@alignCast(&@as(u32, @intFromEnum(ptr.descriptor_id))))),
...
pub fn getImageStagingBuffer(size: math.uvec4, format: vk.Format) Error!VKBuffer {
    const texel_size = frmt.formatTexelSize(format);
    return try VKBuffer.create(
        @reduce(.Mul, size) * texel_size[0] / texel_size[1],
        .{ .transfer_src = true },
        .map,
    );
}
...
pub fn updateDescriptorSet(
    ...
    sampled_images: []const *VKTexture,
) Error!void {
    _ = .{storage_images};
    descriptor_buffer_info_list.items = descriptor_buffer_info_list.items[0..0];
    descriptor_image_info_list.items = descriptor_image_info_list.items[0..0];
    device.updateDescriptorSets(bindless_descriptor_types.len, &blk: {
        ...
        inline for (&write_descriptor_sets, &bindless_descriptor_types, 0..) |*wd, dt, i| {
            switch (dt) {
                .storage_buffer => for (storage_buffers, 0..) |b, j| if (b.usage.storage) {
                    b.descriptor_id = @enumFromInt(j);
                ...
                .combined_image_sampler => for (sampled_images, 0..) |s, j| if (s.sampler != .null_handle) {
                    s.descriptor_id = @enumFromInt(j);
                    try descriptor_image_info_list.append(arena.allocator(), .{
                        .sampler = s.sampler,
                        .image_view = s.view,
            ...
            wd.* = .{
                ...
                .dst_array_element = 0,
                .descriptor_count = @intCast(switch (dt) {
                    .storage_buffer => storage_buffers.len,
                    .combined_image_sampler => sampled_images.len,
                    else => comptime unreachable,
                }),
                .descriptor_type = dt,
                .p_buffer_info = descriptor_buffer_info_list.items.ptr,
                .p_image_info = if (dt == .combined_image_sampler) descriptor_image_info_list.items.ptr else &.{},
                .p_texel_buffer_view = &.{},
            };
        }
...
pub fn allocateCommandBufferHandleCount(thread_id: ThreadID, queue_type: QueueType, count: u32) Error!VKCommandBuffer {
    const pool = try getCommandPool(thread_id, queue_type);
    var handles: [mfif]vk.CommandBuffer = @splat(.null_handle);
    device.allocateCommandBuffers(&.{
        .command_pool = pool,
        .level = .primary,
        .command_buffer_count = @min(mfif, count),
    }, &handles) catch |err| return wrapMemoryErrors(err);
    return .{ .handles = handles, .queue_type = queue_type };
}
pub inline fn allocateCommandBuffer(thread_id: ThreadID, queue_type: QueueType) Error!VKCommandBuffer {
    return try allocateCommandBufferHandleCount(thread_id, queue_type, mfif);
}
...
// fn queueSubmit(cmds: []const *VKCommandBuffer, queue_type: QueueType) Error!void {
// pub fn queueSubmit(cmd: *VKCommandBuffer, queue_type: QueueType) Error!void {
//     device.queueSubmit2(queue(queue_type), 1, &.{.{
...
//     }}, .nulL_handle) catch |err| return wrapMemoryErrors(err);
// }
...
pub const QFI = u8;
pub inline fn wrapMemoryErrors(err: anyerror) Error {
...
pub const VKTexture = @import("VKTexture.zig");
pub const DeviceAllocation = device_allocator.DeviceAllocation;
pub const MemoryType = device_allocator.MemoryType;
pub const allocateDeviceMemory = device_allocator.allocateDeviceMemory;
...
pub fn getVulkanFormat(format: Format, feature_flags: vk.FormatFeatureFlags, layout: FormatUsageLayout, not_found_format: vk.Format) vk.Format {
    return for (frmt.compatibleFormats(format)) |vkf| {
        ...
        if (flags.contains(feature_flags)) break vkf;
    } else not_found_format;
}
